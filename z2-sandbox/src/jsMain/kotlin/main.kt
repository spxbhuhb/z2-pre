...
fun main() {
    adaptive(AdaptiveDOMAdapter(window.document.body!!).also { it.trace = true }) {
        ...
        var counter = 0
        //      val time = poll(10.seconds, default = Clock.System.now()) { Clock.System.now() }
        val states = poll(10.seconds, default = emptyList()) { mock(counter) }
...
//            debug(counter)
        ...
        for (state in states) { // closure index: 2
            machineState(state)
        }
//        }
