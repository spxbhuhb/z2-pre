...
import hu.simplexion.z2.xlsx.internal.dom.toXml
import hu.simplexion.z2.xlsx.internal.model.*
import hu.simplexion.z2.xlsx.model.XlsxCell
...
private fun XlsxCell.toDom(sharedStrings: SharedStrings): Cell? {
    ...
    return when (val v = value) {
        ...
        is Enum<*> -> {
            val str = if (conf.localizedEnums) v.name.localized.value else v.name
        ...
        is Boolean -> {
            if (conf.localizedBooleans) {
                val sharedStringId = sharedStrings.addString(v.localized)
                Cell(coord, sharedStringId, Cell.Type.SHARED_STRING, numberFormat.xfId)
            } else {
