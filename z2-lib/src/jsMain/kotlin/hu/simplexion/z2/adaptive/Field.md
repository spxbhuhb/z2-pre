...
The configurations are typically set during the declaration of the field, rarely changes afterward.

The plugins are functions that change field behaviour: data retrieval, validation, rendering.

The generic `FieldState` and `FieldConfig` classes are used to store data common for all input fields.
