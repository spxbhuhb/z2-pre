package hu.simplexion.z2.devutil.kotlin.services
...
class PluginAnnotationsProvider(testServices: TestServices) : EnvironmentConfigurator(testServices) {
    ...
    companion object {
        private const val ANNOTATIONS_JAR_DIR = "../../z2-site/build/libs/"
        private val ANNOTATIONS_JAR_FILTER = FilenameFilter { _, name ->
            name.startsWith("z2-site-") && name.endsWith("-all.jar") && "sources" !in name
        }
    }
    ...
    private val failMessage = { "Runtime JAR does not exist. Please run z2-site:shadowJar" }
}