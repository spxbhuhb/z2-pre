...
class PluginAnnotationsProvider(testServices: TestServices) : EnvironmentConfigurator(testServices) {
    ...
    companion object {
        private const val ANNOTATIONS_JAR_DIR = "../z2-commons-runtime/build/libs/"

        private val ANNOTATIONS_JAR_FILTER = FilenameFilter { _, name ->
            name.startsWith("z2-commons-runtime-") && name.endsWith("-all.jar") && "sources" !in name
        }
    }
    ...
    private val failMessage = { "Runtime JAR does not exist. Please run :z2-commons-runtime:shadowJar" }
}