...
class AdaptiveClassSymbols(
    ...
    init {
        ...
        if (! ::externalPatch.isInitialized) {
            throw AdaptiveCompilationException(ADAPTIVE_IR_INVALID_EXTERNAL_CLASS, additionalInfo = "missing ${Strings.ADAPTIVE_EXTERNAL_PATCH_PROP} property")
        }
    ...
    private fun function(invalidate: MutableList<IrSimpleFunction>, it: IrSimpleFunction) {
        if (it.name.identifier.startsWith(Strings.ADAPTIVE_INVALIDATE_FUN)) {
            invalidate += it
            return
        }
    ...
    private fun property(indices: List<String>, it: IrProperty) {
        ...
        if (name == Strings.ADAPTIVE_EXTERNAL_PATCH_PROP) {
            externalPatch = it
            externalPatchGetter = it.getter ?: throw AdaptiveCompilationException(ADAPTIVE_IR_INVALID_EXTERNAL_CLASS, additionalInfo = "missing ${Strings.ADAPTIVE_EXTERNAL_PATCH_PROP} getter")
            return
        }
