...
import hu.simplexion.z2.kotlin.schematic.SCHEMATIC_COMPANION_SET_FIELD_VALUE
...
class SetFieldValue(
    ...
    override fun IrSimpleFunction.buildBody() {
        body = DeclarationIrBuilder(irContext, this.symbol).irBlockBody {
            + irWhen(
                ...
                it.branches += irElseBranch(throwSchemaFieldNotFound(transformedClass, irGet(this@buildBody.valueParameters[hu.simplexion.z2.kotlin.schematic.ir.companion.SetFieldValue.Companion.ARG_FIELD_NAME])))
            }
        }
    }
    ...
    fun IrBlockBodyBuilder.toBranch(visitor: SchematicFieldVisitor, function: IrSimpleFunction): IrBranch {
        ...
        return irBranch(
            irEqual(irConst(property.name.identifier), irGet(function.valueParameters[hu.simplexion.z2.kotlin.schematic.ir.companion.SetFieldValue.Companion.ARG_FIELD_NAME])),

            if (property.setter != null) {
                irCall(
                    property.setter !!.symbol,
                    dispatchReceiver = irGet(function.valueParameters[hu.simplexion.z2.kotlin.schematic.ir.companion.SetFieldValue.Companion.ARG_INSTANCE]),
                ).also {
                    it.putValueArgument(0, irImplicitCast(irGet(function.valueParameters[hu.simplexion.z2.kotlin.schematic.ir.companion.SetFieldValue.Companion.ARG_VALUE]), property.getter !!.returnType))
                }
            } else {
                throwSchemaFieldNotFound(transformedClass, irGet(function.valueParameters[hu.simplexion.z2.kotlin.schematic.ir.companion.SetFieldValue.Companion.ARG_FIELD_NAME]))
            }
        )
    }
