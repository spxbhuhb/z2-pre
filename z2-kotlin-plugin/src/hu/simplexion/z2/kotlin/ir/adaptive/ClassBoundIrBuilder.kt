...
open class ClassBoundIrBuilder(
    ...
    fun builder(startOffset: Int): IrSimpleFunction =
        irFactory.buildFun {
            name = Name.identifier("$ADAPTIVE_BUILDER$startOffset")
            ...
            function.addValueParameter(Strings.ADAPTIVE_BUILDER_PARENT, classBoundFragmentType)
    ...
    fun externalPatch(startOffset: Int): IrSimpleFunction =
        irFactory.buildFun {
            name = Name.identifier("$ADAPTIVE_EXTERNAL_PATCH_OF_CHILD$startOffset")
            returnType = irBuiltIns.unitType
            returnType = irBuiltIns.unitType
            modality = Modality.OPEN
        }.also { function ->
    ...
    fun irTraceDirect(dispatchReceiver: IrExpression, point: String, parameters: List<IrExpression>): IrStatement {
        return IrCallImpl(
            ...
            ADAPTIVE_TRACE_ARGUMENT_COUNT,
        ).also {
            it.dispatchReceiver = dispatchReceiver
            it.putValueArgument(ADAPTIVE_TRACE_ARGUMENT_NAME, irConst(irClass.name.identifier))
            it.putValueArgument(ADAPTIVE_TRACE_ARGUMENT_POINT, irConst(point))
            it.putValueArgument(ADAPTIVE_TRACE_ARGUMENT_DATA, buildTraceVarArg(parameters))
        }
    }
