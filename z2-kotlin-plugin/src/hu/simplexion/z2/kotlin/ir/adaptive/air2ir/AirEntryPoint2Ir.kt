...
class AirEntryPoint2Ir(
    ...
    fun toIr() {
        ...
        function.body = DeclarationIrBuilder(irContext, function.symbol).irBlockBody {
            ...
            +irCall(
                ADAPTIVE_CREATE.function.symbol,
                dispatchReceiver = irGet(root)
            )

            +irCall(
                ADAPTIVE_MOUNT.function.symbol,
                dispatchReceiver = irGet(root),
                args = arrayOf(
                    irCall(
                        this@AirEntryPoint2Ir.context.adaptiveAdapterClass.getPropertyGetter(ADAPTIVE_ROOT_BRIDGE)!!.owner.symbol,
