...
import hu.simplexion.z2.kotlin.ir.adaptive.diagnostics.ErrorsAdaptive.ADAPTIVE_IR_INVALID_RENDERING_STATEMENT
import hu.simplexion.z2.kotlin.ir.adaptive.diagnostics.ErrorsAdaptive.ADAPTIVE_IR_RENDERING_INVALID_DECLARATION
import hu.simplexion.z2.kotlin.ir.adaptive.diagnostics.ErrorsAdaptive.RIU_IR_RENDERING_NON_ADAPTIVE_CALL
import hu.simplexion.z2.kotlin.ir.adaptive.rum.*
import hu.simplexion.z2.kotlin.ir.adaptive.util.AdaptiveAnnotationBasedExtension
import hu.simplexion.z2.kotlin.ir.adaptive.util.isParameterCall
...
class IrFunction2Rum(
    ...
    override fun getAnnotationFqNames(modifierListOwner: KtModifierListOwner?): List<String> =
        rumClass.adaptiveContext.annotations

    fun IrElement.dependencies(): List<RumStateVariable> {
        val visitor = DependencyVisitor(adaptiveContext, rumClass)
        accept(visitor, null)
        return visitor.dependencies
    }

    fun transform(): RumClass {
        rumClass = RumClass(adaptiveContext, irFunction, scope)

        StateDefinitionTransform(rumClass, skipParameters).transform()

        transformRoot()

        adaptiveContext.rumClasses += rumClass
            ...
            when (statement) {
                is IrBlock -> {
                    when (statement.origin) {
                        ...
                        else -> ADAPTIVE_IR_INVALID_RENDERING_STATEMENT.report(rumClass, statement)
                    }
                }
    ...
    fun transformDeclaration(declaration: IrDeclaration, origin: RumDeclarationOrigin): RumDeclaration? =
        when (declaration) {
            ...
            else -> ADAPTIVE_IR_RENDERING_INVALID_DECLARATION.report(rumClass, declaration)
        }
    ...
    fun transformCall(statement: IrCall): RumCall? {
        ...
        if (! statement.isAnnotatedWithAdaptive()) {
            return RIU_IR_RENDERING_NON_ADAPTIVE_CALL.report(rumClass, statement)
        }
    ...
    fun transformHigherOrderArgument(index: Int, expression: IrFunctionExpression): RumExpression =
        RumHigherOrderArgument(
            IrFunction2Rum(adaptiveContext, expression.function, 0, rumClass).transform(),
    ...
    fun transformWhen(statement: IrWhen, subject: IrVariable? = null): RumWhen? {
        val adaptiveWhen = RumWhen(rumClass, blockIndex, subject, statement)
    ...
    fun transformRenderingExpression(expression: IrExpression): RumRenderingStatement? {
        return when (expression) {
            ...
            is IrWhen -> transformWhen(expression)
                ...
                ADAPTIVE_IR_RENDERING_INVALID_DECLARATION.report(rumClass, expression)
                null
            }
        }
    }
