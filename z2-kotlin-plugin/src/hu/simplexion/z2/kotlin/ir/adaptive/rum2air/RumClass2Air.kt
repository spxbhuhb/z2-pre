...
class RumClass2Air(
    ...
    fun toAir(): AirClass {
        ...
        val adapter = addPropertyWithConstructorParameter(ADAPTIVE_ADAPTER.name, classBoundAdapterType, overridden = context.adaptiveAdapter)
        val closure = addPropertyWithConstructorParameter(ADAPTIVE_CLOSURE.name, classBoundClosureType.makeNullable(), overridden = context.adaptiveClosure)
        val parent = addPropertyWithConstructorParameter(ADAPTIVE_PARENT.name, classBoundFragmentType.makeNullable(), overridden = context.adaptiveParent)
        val externalPatch = addPropertyWithConstructorParameter(ADAPTIVE_EXTERNAL_PATCH.name, classBoundExternalPatchType, overridden = context.adaptiveExternalPatch)

        val fragment = addIrProperty(ADAPTIVE_FRAGMENT.name, context.adaptiveFragmentType, inIsVar = false, overridden = context.adaptiveFragment)
    ...
    private fun create(): IrSimpleFunction =
        ...
        irFactory.buildFun {
            name = Name.identifier(ADAPTIVE_CREATE)
    ...
    private fun mount(): IrSimpleFunction =
        ...
        irFactory.buildFun {
            name = Name.identifier(ADAPTIVE_MOUNT)
            returnType = irBuiltIns.unitType
            modality = Modality.OPEN
            isFakeOverride = true
        }.also { function ->
    ...
    private fun unmount(): IrSimpleFunction =
        ...
        irFactory.buildFun {
            name = Name.identifier(ADAPTIVE_UNMOUNT)
            returnType = irBuiltIns.unitType
            modality = Modality.OPEN
            isFakeOverride = true
        }.also { function ->
    ...
    private fun dispose(): IrSimpleFunction =
        ...
        irFactory.buildFun {
            name = Name.identifier(ADAPTIVE_DISPOSE)
            returnType = irBuiltIns.unitType
            modality = Modality.OPEN
            isFakeOverride = true
        }.also { function ->
