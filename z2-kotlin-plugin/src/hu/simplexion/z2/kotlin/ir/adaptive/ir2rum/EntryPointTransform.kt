...
import hu.simplexion.z2.kotlin.ir.adaptive.ADAPTIVE_FQN_ENTRY_FUNCTION
...
/**
 * Creates a [RumClass] and a [RumEntryPoint] for each call of the `adaptive` function (defined in the runtime).
 */
class EntryPointTransform(
    ...
    override fun visitCall(expression: IrCall): IrExpression {
        if (expression.symbol.owner.kotlinFqName != ADAPTIVE_FQN_ENTRY_FUNCTION) {
        ...
        val rumClass = IrFunction2Rum(adaptiveContext, function, skipParameters = 1).transform()

        RumEntryPoint(rumClass, function).also {
            adaptiveContext.rumEntryPoints += it
        }
